...
/**
 * This class consists of password encrypt and decrypt methods
 * 
 * @author kakur
 * 
 */
public class password_Encrypt {
	...
	public static String encryptXOR(String message, String key) {
		...
		try {
			...
			mesg = null;
			keys = null;
			return new String(new BASE64Encoder().encodeBuffer(new String(
					newmsg).getBytes()));
	...
	public static String decryptXOR(String message, String key) {
		try {
			...
			char[] keys = key.toCharArray();
			char[] mesg = new String(decoder.decodeBuffer(message))
					.toCharArray();
